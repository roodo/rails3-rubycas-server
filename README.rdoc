...
by www.roodo.com ( http://www.roodo.com )
...
* gem install rails ( > 3.0.3 )
* gem install mysql2
* gem install crypt-isaac ( ~> 0.9.1 )
* gem install gettext ( ~> 2.1.0 )
...
Add following line to the lib/authenticators/sql_encrypted.rb
  require 'digest/md5'  
Modify your 'encrypt_function' in configuration file (by default located in /config/cas.yml).
  encrypt_function: 'user.encrypted_password == Digest::MD5.hexdigest("#{user.encryption_salt}::#{@password}")'

== LICENSE:

  Apache License 2.0

  Copyright (c) 2010, JUT Living Development Co., Ltd
