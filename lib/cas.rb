...
module CASServer::CAS
  ...
  def generate_proxy_granting_ticket(pgt_url, st)
    ...
    https.start do |conn|
      ...
      if %w(200 202 301 302 304).include?(response.code)
        ...
        pgt.save!
        Rails.logger.debug "PGT generated for pgt_url '#{pgt_url}': #{pgt.inspect}"
        pgt
      else
        Rails.logger.warn "PGT callback server responded with a bad result code '#{response.code}'. PGT will not be stored."
        nil
      end
    end
  end
  ...
  def validate_ticket_granting_ticket(ticket)
    Rails.logger.debug("Validating ticket granting ticket '#{ticket}'")

    if ticket.nil?
      error = "No ticket granting ticket given."
      Rails.logger.debug error
    elsif tgt = TicketGrantingTicket.find_by_ticket(ticket)
      if CasConf[:expire_sessions] && Time.now - tgt.created_on > CasConf[:ticket_granting_ticket_expiry]
        error = "Your session has expired. Please log in again."
        Rails.logger.info "Ticket granting ticket '#{ticket}' for user '#{tgt.username}' expired."
      else
        Rails.logger.info "Ticket granting ticket '#{ticket}' for user '#{tgt.username}' successfully validated."
      end
    else
  ...
  def validate_service_ticket(service, ticket, allow_proxy_tickets = false)
    ...
    if service.nil? or ticket.nil?
      ...
      if st.consumed?
        ...
        Rails.logger.info("Ticket '#{ticket}' for service '#{service}' for user '#{st.username}' successfully validated.")
      end
    else
  ...
  def validate_proxy_granting_ticket(ticket)
    if ticket.nil?
      ...
      if pgt.service_ticket
        Rails.logger.info("Proxy granting ticket '#{ticket}' belonging to user '#{pgt.service_ticket.username}' successfully validated.")
      else
        error = Error.new(:INTERNAL_ERROR, "Proxy granting ticket '#{ticket}' is not associated with a service ticket.")
        Rails.logger.error("#{error.code} - #{error.message}")
      end
    else
    ...
    begin
      http.start do |conn|
        ...
        if response.kind_of? Net::HTTPSuccess
          Rails.logger.info "Logout notification successfully posted to #{st.service.inspect}."
          return true
        else
          Rails.logger.error "Service #{st.service.inspect} responed to logout notification with code '#{response.code}'!"
          return false
        end
      end
    rescue Exception => e
      Rails.logger.error "Failed to send logout notification to service #{st.service.inspect} due to #{e}"
      return false
    end
  end
