...
class CASServer::Authenticators::SQLAuthlogic < CASServer::Authenticators::SQL
  ...
  def validate(credentials)
    ...
    begin
      ...
      Rails.logger.warn("Could not initialize Authlogic crypto class for '#{@options[:encryptor]}'")
    ...
    if results.size > 0
      Rails.logger.warn("Multiple matches found for user '#{@username}'") if results.size > 1
      user = results.first
